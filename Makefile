...
console-dev:
	_build/dev/rel/cinn/bin/cinn console

rel-prod:
	$(SED) -i 's/{cinn, "cinn-version"}/{cinn, "$(VER)"}/g' ./rebar.config
	$(PWD)/script/rebar3 as prod release
	$(PWD)/script/rebar3 as prod tar
	$(SED) -i 's/{cinn, "$(VER)"}/{cinn, "cinn-version"}/g' ./rebar.config
    #$(SCP) -P 8522 $(PWD)/_build/prod/rel/cinn/cinn-$(VER).tar.gz $(FS)
	@printf "\nApplication: %s\n" $(PWD)/_build/prod/rel/cinn/cinn-$(VER).tar.gz
...
rel-stage:
	...
	$(SED) -i 's/{cinn, "$(VER)"}/{cinn, "cinn-version"}/g' ./rebar.config
    #$(SCP) -P 8522 $(PWD)/_build/stage/rel/cinn/cinn-$(VER).tar.gz $(FS)
	@printf "\nApplication: %s\n" $(PWD)/_build/stage/rel/cinn/cinn-$(VER).tar.gz
